...
def create_booking_procedure(cursor):
    ...
    begin
        if rNum not in (select regNum from Bookings) and sID not in (select spaceID from Bookings) and uID not in (select userID from Bookings) then
            if sID in (select spaceID from Floors) and fNum in (select floorNum from Floors) then
                insert into Bookings (regNum, spaceID, userID) values (rNum, sID, uID);
            end if;
        end if;
    end;
    '''
...
def create_floor_trigger(cursor):
    ...
    begin
        ...
        if new.spaceID in (select spaceID from Bookings where spaceID = new.spaceID and regNum = new.parkedRegNum) then
            update Bookings set startTime = NOW() where spaceID = new.spaceID;
        end if;
    end;
    '''
...
def main():
    ...
    ini = input("\nDo you want to initialize/reset database (y/yes or n/no): ")
    if ini.lower() == "y" or ini.lower() == "yes":
    ...
    while intro:
        ...
        if choice == "1": #sign in
            ...
            if choice == "1":
                ...
                if cursor.fetchall() == []:
                    print("\nParked on wrong space and got asked to leave!")
                else:
                    print("\nParked on a space reserved for this register number.")
                ...
                if cursor.fetchall() != []:
                    ...
                    print("\nYou were not parked so nothing was unbooked.")
            else:
                print("Wrong input!")
        ...
        if choice == "1": #See free park spaces
            ...
            if result[0][0] == 0:
                ...
                if result[0][0] == 0:
                    ...
                    #Not successful
                    if result == []:
                        print("-> Something was wrong and space could not be reserved! (Remember only one reservation can be made at a time) <-")
